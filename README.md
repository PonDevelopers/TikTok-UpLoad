...
A hosted API service for automated TikTok video uploads — supports music, affiliate products, caption/hashtag, account info, affiliate dashboard, monetization wallet, product management, and a ready-to-use upload bot with Telegram notifications.

> **API endpoint:** `http://lechaukhaapi.eu.org`  
...
> ⚠️ After 10 failed attempts with an invalid key, the IP is blocked for 24 hours.
...
**cURL:**
```bash
curl -X POST http://lechaukhaapi.eu.org/account \
...
r = requests.post(
    "http://lechaukhaapi.eu.org/account",
...
**cURL:**
```bash
curl -X POST http://lechaukhaapi.eu.org/affiliate \
  -H "X-API-Key: your_key_here" \
  -H "Content-Type: application/json" \
  -d '{"cookies": "sessionid=abc;msToken=xyz"}'
```
...
r = requests.post(
    "http://lechaukhaapi.eu.org/affiliate",
    headers={"X-API-Key": "your_key_here"},
    json={"cookies": "sessionid=abc;msToken=xyz;..."},
)
print(r.json())
...
### `POST /monetization` — Get monetization wallet

Returns the creator's wallet balance and income breakdown by program for the last 7 days.
...
```json
{
  "success": true,
  "data": {
    "wallet_balance": "₫96,421.00",
    "seven_d_income": "₫105,973.00",
    "programs": [
      { "name": "TikTok Shop for Creator", "seven_d": "₫105.97K" },
      { "name": "Video Gifts",             "seven_d": "₫0.00"    },
      { "name": "Creator Marketplace",     "seven_d": "₫0.00"    }
    ]
  }
}
```
...
**cURL:**
```bash
curl -X POST http://lechaukhaapi.eu.org/monetization \
  -H "X-API-Key: your_key_here" \
  -H "Content-Type: application/json" \
  -d '{"cookies": "sessionid=abc;msToken=xyz"}'
```
...
r = requests.post(
    "http://lechaukhaapi.eu.org/monetization",
    headers={"X-API-Key": "your_key_here"},
    json={"cookies": "sessionid=abc;msToken=xyz;..."},
)
print(r.json())
...
> ⚠️ Only works for accounts enrolled in TikTok Creator Monetization programs.
...
**cURL:**
```bash
curl -X POST http://lechaukhaapi.eu.org/music \
  -H "X-API-Key: your_key_here" \
  -H "Content-Type: application/json" \
...
r = requests.post(
    "http://lechaukhaapi.eu.org/music",
    headers={"X-API-Key": "your_key_here"},
...
**cURL:**
```bash
curl -X POST http://lechaukhaapi.eu.org/product \
  -H "X-API-Key: your_key_here" \
  -H "Content-Type: application/json" \
...
r = requests.post(
    "http://lechaukhaapi.eu.org/product",
    headers={"X-API-Key": "your_key_here"},
...
### `POST /add_product` — Check and add product to creator showcase

Checks whether a product can be added, then adds it automatically if possible. Returns full product details and the result of the add operation.
...
| `already_added` | Product was already in your showcase |
| `cannot_add` | Product cannot be added (ineligible) |
| `failed` | Add request was sent but TikTok rejected it |

**cURL:**
```bash
curl -X POST http://lechaukhaapi.eu.org/add_product \
  -H "X-API-Key: your_key_here" \
  -H "Content-Type: application/json" \
  -d '{"cookies": "sessionid=abc;...", "product_id": "1730987654321"}'
```
...
r = requests.post(
    "http://lechaukhaapi.eu.org/add_product",
    headers={"X-API-Key": "your_key_here"},
    json={"cookies": "sessionid=abc;...", "product_id": "1730987654321"},
)
print(r.json())
...
**cURL — basic upload:**
```bash
curl -X POST http://lechaukhaapi.eu.org/upload \
  -H "X-API-Key: your_key_here" \
...
r = requests.post(
    "http://lechaukhaapi.eu.org/upload",
    headers={"X-API-Key": "your_key_here"},
    data={
        "cookies":  "sessionid=abc;msToken=xyz;...",
...
| `Too many failed attempts. Blocked 24h.` | 10+ wrong key attempts from same IP |
...
| `Monetization API error` | Account is not enrolled in any monetization program |
| `Check product failed` | Product ID invalid or not accessible |
...
On first run, the bot walks you through the full setup interactively. Set your API key in `Api.py` before running:
...
| `/info` | List accounts, reply with a number to see full profile, affiliate revenue, and monetization wallet |
| `/add_product <id>` | Add a product to all accounts and show price, commission, stock, and result |
...
Dịch vụ API upload TikTok tự động — hỗ trợ nhạc, sản phẩm affiliate, caption/hashtag, thông tin tài khoản, dashboard affiliate, ví kiếm tiền (monetization), quản lý sản phẩm, và bot upload sẵn có với thông báo Telegram.
...
> ⚠️ Sau 10 lần nhập sai key, IP sẽ bị block 24 giờ.
...
### `POST /monetization` — Lấy thông tin ví kiếm tiền

Trả về số dư ví và thu nhập theo từng chương trình kiếm tiền trong 7 ngày gần nhất.
...
> ⚠️ Chỉ hoạt động với tài khoản đã tham gia các chương trình kiếm tiền của TikTok Creator.
...
### `POST /add_product` — Kiểm tra và thêm sản phẩm vào showcase

Kiểm tra xem sản phẩm có thể thêm không, sau đó tự động thêm nếu hợp lệ. Trả về đầy đủ thông tin sản phẩm và kết quả thêm.
...
Các giá trị của `add_result`:

| Giá trị | Ý nghĩa |
|---|---|
| `success` | Thêm sản phẩm thành công |
| `already_added` | Sản phẩm đã có trong showcase rồi |
| `cannot_add` | Sản phẩm không đủ điều kiện để thêm |
| `failed` | Gửi request thành công nhưng TikTok từ chối |
...
| `Too many failed attempts. Blocked 24h.` | Nhập sai key 10 lần từ cùng IP |
...
| `Monetization API error` | Tài khoản chưa tham gia chương trình kiếm tiền nào |
| `Check product failed` | product_id không hợp lệ hoặc không truy cập được |
...
| `/info` | Liệt kê tài khoản, gõ số để xem chi tiết: profile, doanh thu affiliate và ví kiếm tiền |
| `/add_product <id>` | Thêm sản phẩm vào tất cả tài khoản, hiển thị giá, hoa hồng, tồn kho và kết quả |
